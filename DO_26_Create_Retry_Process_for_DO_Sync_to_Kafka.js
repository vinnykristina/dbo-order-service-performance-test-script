...
export default async function () {
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        const urlcreateRetryProcessForDO = `https://moonlay-api.dbo.dev/order-srv/delivery-orders/retry-to-sync-kafka/${csvData[i].doEventLogId}`
        // console.log(urlDoById);

        group('Create Retry Process For DO Sync To Kafka', function () {
            const createRetryProcessForDO = http.request('GET', urlcreateRetryProcessForDO, "", {
                headers: {
