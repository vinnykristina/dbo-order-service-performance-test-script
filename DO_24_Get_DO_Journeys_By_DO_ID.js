...
export default async function () {
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        const urlDoJourneysByDoId = `https://moonlay-api.dbo.dev/delivery-orders/${csvData[i].id}/journeys`
        // console.log(urlDoById);

        group('Get DO Journeys By Delivery Order ID', function () {
            const getDoJourneysByDoId = http.request('GET', urlDoJourneysByDoId,"", {
                headers: {
