...
export default async function () {
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        const urlgetDoUploadItemErrByReqId = `https://moonlay-api.dbo.dev/delivery-orders/upload-histories-by-request-id/${csvData[i].request_id}/error-items`
        // console.log(urlDoById);

        group('Get DO Upload Item Errors By Request ID', function () {
            const getDoUploadItemErrByReqId = http.request('GET', urlgetDoUploadItemErrByReqId, '', {
                headers: {
