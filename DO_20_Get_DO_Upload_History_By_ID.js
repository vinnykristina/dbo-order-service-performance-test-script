...
export default async function () {
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        const urlDoUploadHisById = `https://moonlay-api.dbo.dev/delivery-orders/upload-histories/${csvData[i].request_id}`
        // console.log(urlDoById);

        group('Get DO Upload History By ID', function () {
            const getDoUploadHisById = http.request('GET', urlDoUploadHisById, "", {
                headers: {
